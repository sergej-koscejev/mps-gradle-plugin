...
- `MpsMigrate`: pass two additional `--add-opens` arguments to MPS to support version 2025.2 and above.
...
- Removes deprecated usage of `Project.exec` with an injected `ExecOperations` instead, see https://docs.gradle.org/current/userguide/upgrading_version_8.html#deprecated_project_exec
- Removes another deprecated usage of `Task.getProject()` to be compatible with the Gradle configuration cache, see https://docs.gradle.org/current/userguide/upgrading_version_8.html#task_project
